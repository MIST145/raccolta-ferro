...
function makeProp(data, freeze, synced)
    ...
    SetEntityHeading(prop, data.coords + 180.0) -- Correzione qui
...
AddEventHandler("raccoltaferro", function()
    ...
    if lib.progressBar({
        duration = Config.Timings["Estrattore"],
        label = 'Raccogliendo...',
        useWhileDead = false,
        canCancel = false,
        disable = {
...
AddEventHandler("gngn:showferrocontext", function()
        lib.registerContext({
            id = 'ferro',
            title = 'Ferraio ' .. GetPlayerName(PlayerId()) .. '(' .. GetPlayerServerId(PlayerId()) .. ')',
            options = {
                {
                    title = 'Accessori per il Ferro',
                    description = 'Acquista qui i tuoi picconi',
                    icon = 'fas fa-toolbox',
                    menu = 'ferro2',
                    iconColor = '#0a77ae',
                    arrow = true,
...
lib.registerContext({
    ...
    options = {
        {
            title = 'Sega elettrica',
            description = 'Con questa puoi tagliare tutti i pezzi di ferro. Costo 500$',
